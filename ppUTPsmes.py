...
def runstar(gaiaid=None,outputname=None, version='V0', **kwargs):
    ...
    # grab data
    data = getdata()
    phot_filtarr = list(data['phot'].keys())
    ...
    if outputname is None:
        ...
        samplefile = f'./samples/samples_demo_UTPsmes_{version}.fits'
    else:
        # set the output file name
    ...
    photmod_p = [photmod_p[xx] for xx in phot_filtarr]

    photpars_s = ([
        bfdict['Teff_s'][0],bfdict['log(g)_s'][0],bfdict['[Fe/H]_s'][0],bfdict['[a/Fe]_s'][0],
        bfdict['log(R)_s'][0],bfdict['dist'][0],bfdict['Av'][0],3.1])
    photmod_s = genphotfn(photpars_s)
    photmod_s = [photmod_s[xx] for xx in phot_filtarr]
    ...
    photmod_est = {x:photmod_est[ii] for ii,x in enumerate(phot_filtarr)}
    ...
    # define output file
    outfile = f'./plots/compmod_demo_sviTPsmes_{version}.pdf'
